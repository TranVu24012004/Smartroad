...
app.post("/predict", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    pythonProcess.on("close", async (code) => {
      ...
      console.log("Full output:", predictionResult);
      ...
      const result = lastLine.split(" ")[0] || "Unknown";
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    // IMPORTANT: Special case for admin login
    ...
    console.log(`User ${email} authenticated successfully. Role: ${userRole}`);
    
    // For admin/authority users, create a special ID with prefix
    const userId = isAdminAccount 
      ? "admin_" + user._id.toString() 
      : user._id.toString();

    // Return user ID, name and ROLE for client-side storage
    res.status(200).json({ 
